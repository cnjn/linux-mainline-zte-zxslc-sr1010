...
- Recovered signature: `int zte_gephy_get_1000m_tx_dac_lv(u8 phy, u16 *level)`.

## Semantics

The function rejects a null output pointer, logs `"tx_dac_lv is null\n"`, and
returns `-1`. Otherwise it writes `0xffffb407` to MDIO register 16, reads
...
- Complete ARM64 body at `0x1ada0` through `0x1adfc`.
- Null branch and exact log string.
- Register-16 selection value `0xffffb407`, register-17 `0x3f` mask, halfword
  output store, and zero/-1 returns.
- Exhaustive direct xref query reported no code references.
- IDA type at `0x1ada0` updated to the recovered signature.
