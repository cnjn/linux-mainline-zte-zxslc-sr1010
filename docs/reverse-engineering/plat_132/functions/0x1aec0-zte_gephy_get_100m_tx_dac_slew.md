...
- Recovered signature: `int zte_gephy_get_100m_tx_dac_slew(u8 phy, u16 *slew)`.

## Semantics

The function rejects a null output pointer, logs `"tx_dac_slew is null\n"`, and
returns `-1`. Otherwise it writes `0xffffb408` to MDIO register 16, reads
...
- Complete ARM64 body at `0x1aec0` through `0x1af1c`.
- Null branch and exact log string.
- Register-16 selection value `0xffffb408`, register-17 `0x7` mask, halfword
  output store, and zero/-1 returns.
- Exhaustive direct xref query reported no code references.
- IDA type at `0x1aec0` updated to the recovered signature.
