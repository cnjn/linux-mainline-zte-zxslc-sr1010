...
- Recovered signature: `int zte_gephy_set_1000m_tx_dac_lv(u8 phy, u16 level)`.
...
Otherwise it selects MDIO register 17 by writing `0xffffb407` to register 16,
...
- Complete ARM64 body at `0x1b218` through `0x1b288`.
- `UXTH` level conversion, unsigned comparison to `0x3f`, exact error string,
  selector value `0xffffb407`, and `0xffc0` RMW mask.
- Exhaustive direct xref query reported no code references.
- IDA type at `0x1b218` updated to the recovered signature.
