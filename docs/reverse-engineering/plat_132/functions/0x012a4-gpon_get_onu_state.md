...
- Recovered signature: `u32 gpon_get_onu_state(void)`.

## Semantics

Returns the low three bits from the 32-bit word at PON offset `0x94000`.
...
- Complete ARM64 body at `0x12a4` through `0x12b8`.
- Exact offset and `AND #7` extraction.
- Exhaustive direct xref query found two callers.
- IDA type at `0x12a4` updated to the recovered unsigned-return signature.
